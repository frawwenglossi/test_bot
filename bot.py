...
def команда_админ_панель(message):
    ...
    клавиатура.row('🔍 ОТВЕТИТЬ НА ОБРАЩЕНИЕ / ПОСМОТРЕТЬ ОБРАЩЕНИЯ')
...
@bot.message_handler(func=lambda message: message.text == '🔍 ОТВЕТИТЬ НА ОБРАЩЕНИЕ / ПОСМОТРЕТЬ ОБРАЩЕНИЯ')
...
def обработать_текст(message):
    ...
    if chat_id in состояния and состояния[chat_id] == 'отвечает':
        ...
        telegram_id = db.посмотреть_одно_обращение(номер_и_обращение[chat_id])[1]
